...
$(".modifyVersion").on("click",function(){
	...
	if(status == "4" || status == "6"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href="appversionmodify?vid="+ versionid + "&aid="+ appinfoid +"&pageNum="+obj.attr("pageNum");
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status==4 || status == 6){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href="appinfomodify?id="+ obj.attr("appinfoid")+"&pageNum="+ obj.attr("pageNum");
	}else{
