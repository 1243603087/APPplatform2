								...
								<select name="queryStatus" class="form-control" >
									<c:if test="${statusList != null }">
									   ...
									   <c:forEach var="dataDictionary" items="${statusList}">
									   		<option <c:if test="${dataDictionary.id == queryStatus }">selected="selected"</c:if>
								...
								<select name="queryFlatformId" class="form-control">
									<c:if test="${flatFormList != null }">
									   ...
									   <c:forEach var="dataDictionary" items="${flatFormList}">
									   		<option <c:if test="${dataDictionary.id == queryFlatformId }">selected="selected"</c:if>
									   		value="${dataDictionary.id}">${dataDictionary.valueName}</option>
									   </c:forEach>
									</c:if>
        						</select>
							...
							<tbody>
								<c:forEach var="appInfo" items="${appInfoList.list}" varStatus="status">
									<tr role="row" class="odd">
										...
										<td><span id="appInfoStatus${appInfo.id}">${appInfo.softwareStatus }</span></td>
										...
										<div class="btn-group">
											<div class="dropdown">
												<button id="dLabel" class="btn btn-danger dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
													点击操作
												...
												<ul class="dropdown-menu" aria-labelledby="dLabel" role="menu">
													<li>
														<c:choose>
															<c:when test="${appInfo.status == 5 || appInfo.status == 8}">
